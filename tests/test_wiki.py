...
class TestWikiTools(WikiDirTestCase, unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        super().setUp()
    ...
    async def test_update_then_read_through_the_registry(self):
        ...
        read_back = await self.registry.execute(
            "read_wiki_page", json.dumps({"name": "user-prefs"})
        )
        self.assertEqual(read_back, "# Prefs\nConda.")

    async def test_read_missing_page_lists_existing_ones(self):
    ...
    async def test_read_missing_page_on_empty_wiki(self):
        result = await self.registry.execute(
            "read_wiki_page", json.dumps({"name": "nope"})
        )
        self.assertIn("no pages yet", result)

    async def test_errors_come_back_as_strings(self):
        ...
        for args in (
            ...
            result = await self.registry.execute("update_wiki_page", json.dumps(args))
            self.assertIn("Error", result)
